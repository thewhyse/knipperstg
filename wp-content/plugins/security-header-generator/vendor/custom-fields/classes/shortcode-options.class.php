 ...
 *
 */
if ( ! class_exists( 'KPSHG_Shortcoder' ) ) {
  class KPSHG_Shortcoder extends KPSHG_Abstract{
    ...
    public $args         = array(
      ...
      'class'            => '',
      'gutenberg'        => array(
        'title'          => 'KPSHG Shortcodes',
        'description'    => 'KPSHG Shortcode Block',
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
        'keywords'       => array( 'shortcode', 'kpshg', 'insert' ),
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'wp_ajax_kpshg-get-shortcode-'. $this->unique, array( $this, 'get_shortcode' ) );

      if ( ! empty( $this->args['show_in_editor'] ) ) {

        $name = str_replace( '_', '-', sanitize_title( $this->unique ) );

        KPSHG::$shortcode_instances[] = wp_parse_args( array( 'name' => 'kpshg/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( KPSHG::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'KPSHG', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'KPSHG_Field_icon', 'add_footer_modal_icon' ) );
    ...
    public function add_footer_modal_shortcode() {
      ...
      if( ! wp_script_is( 'kpshg' ) ) {
        return;
      }

      $class        = ( $this->args['class'] ) ? ' '. esc_attr( $this->args['class'] ) : '';
      $has_select   = ( count( $this->pre_tabs ) > 1 ) ? true : false;
      $single_usage = ( ! $has_select ) ? ' kpshg-shortcode-single' : '';
      $hide_header  = ( ! $has_select ) ? ' hidden' : '';

    ?>
      <div id="kpshg-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui kpshg-modal kpshg-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'kpshg_shortcode_nonce' ) ); ?>">
          ...
          <div class="kpshg-modal-table-cell">
            ...
            <div class="kpshg-modal-inner">
              <div class="kpshg-modal-title">
                ...
                <div class="kpshg-modal-close"></div>
              </div>
              <?php

                echo '<div class="kpshg-modal-header'. esc_attr( $hide_header ) .'">';
                echo '<select>';
              ...
              <div class="kpshg-modal-insert-wrapper hidden"><a href="#" class="button button-primary kpshg-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'kpshg_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              KPSHG::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'kpshg' );
            $inner_shortcode = ( ! empty( $section['group_shortcode'] ) ) ? $section['group_shortcode'] : $shortcode;

            echo '<div class="kpshg--repeatable">';

              echo '<div class="kpshg--repeat-shortcode">';

                echo '<div class="kpshg-repeat-remove fas fa-times"></div>';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  KPSHG::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="kpshg--repeat-button-block"><a class="button kpshg--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. $button_title .'</a></div>';
        ...
        echo '<div class="kpshg-field kpshg-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'kpshg' ) .'</div>';
      }
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'KPSHG_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( kpshg_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'KPSHG_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'kpshg-gutenberg-block', KPSHG::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'kpshg-gutenberg-block', 'kpshg_gutenberg_blocks', KPSHG::$shortcode_instances );

      foreach ( KPSHG::$shortcode_instances as $block ) {

        register_block_type( $block['name'], array(
          'editor_script' => 'kpshg-gutenberg-block',
        ) );
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( KPSHG::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary kpshg-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. esc_html( $value['button_title'] ) .'</a>';
      }
