 ...
 *
 */
if ( ! class_exists( 'KPSHG_Field_link_color' ) ) {
  class KPSHG_Field_link_color extends KPSHG_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'kpshg' ),
        'hover'    => esc_html__( 'Hover', 'kpshg' ),
        'active'   => esc_html__( 'Active', 'kpshg' ),
        'visited'  => esc_html__( 'Visited', 'kpshg' ),
        'focus'    => esc_html__( 'Focus', 'kpshg' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="kpshg--left kpshg-field-color">';
          echo '<div class="kpshg--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="kpshg-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
