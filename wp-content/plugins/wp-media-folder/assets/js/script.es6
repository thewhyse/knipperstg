    ...
    <div class="wpmf-process-foot"><span>${wpmf.l18n.total_size}&nbsp;</span><span class="wpmf-upload-total-size">0</span></div>
                    ...
                    if (parseInt(wpmf.vars.wpmf_post_type) !== 1 || !$('#wpb_visual_composer').is(":visible")) {
                        if (typeof wp.media.frame !== 'undefined' && typeof wp.media.frame._state !== 'undefined') {
                            let type = file_info.attributes.file.type;
                            let icon = '';
                            ...
                            let item = wpmfFoldersModule.getUploadItemTemplate(file_info.attributes.file.id, file_info.attributes.file.name, icon);
                            // Create the download attachment
         ...
         */
        getBackboneOfMedia: function(c = false) {
            let t, a, n = wpmfFoldersModule.getFrame().parents(".media-modal:visible");
         ...
         */
        reloadAttachments: function (c = false) {
            // Force reloading files
            let n = wpmfFoldersModule.getBackboneOfMedia(c);
        ...
        getFolderRendering: function (type, name, term_id, parent_id) {
            ...
            if (type === 'folder') {
                ...
                if (wpmfFoldersModule.categories[term_id].drive_type !== 'google_drive'
                    ...
                    && wpmfFoldersModule.categories[term_id].drive_type !== 'nextcloud') {
    ...
    <div class="wpmf-process-foot"><span>${wpmf.l18n.total_size}&nbsp;</span><span class="wpmf-upload-total-size">0</span></div>
                        ...
                        if (parseInt(wpmf.vars.wpmf_post_type) !== 1 || !$('#wpb_visual_composer').is(":visible")) {
                            if (typeof wp.media.frame !== 'undefined' && typeof wp.media.frame._state !== 'undefined') {
                                ...
                                let relativePath = first_file.relativePath;
                                let relativePaths = relativePath.split('/');
                                let folder_name = wpmfFoldersModule.string_to_slug(relativePaths[0]);
                                if (!$('.wpmf-process-file-item[data-cid="'+ folder_name +'"]').length) {
                                    let icon = '<span class="wpmf-fileupload-thumbnail material-icons-outlined"> folder </span>';
                                    let item = wpmfFoldersModule.getUploadItemTemplate(folder_name, folder_name, icon);
                                    // Create the download attachment
                                    if ($('.wpmf-process-body ul li').length) {
                                        $('.wpmf-process-body ul').append(item);
                                    } else {
                ...
                wpmfUploader.on('fileSuccess', function (file, res) {
                    try {
                        let index = $('.wpmf-upload-completed').html();
                        if (index == '') {
                            index = 0;
                        }

                        let completed = parseInt(index) + 1;
                        if (completed <= parseInt(wpmfFoldersModule.filesTotal)) {
                            $('.wpmf-upload-completed').text(completed);
                            let percent = (completed)/parseInt(wpmfFoldersModule.filesTotal)*100;
                            ...
                            let folder_name = wpmfFoldersModule.string_to_slug(relativePaths[0]);
                            if (percent == 100) {
                                $('.wpmf-process-file-item[data-cid="'+ folder_name +'"] .wpmf-fileupload-progress-bar').css({'background': 'none'});
                                $('.wpmf-process-file-item[data-cid="'+ folder_name +'"] .wpmf-fileupload-progress-bar').html('<svg class="a-s-fa-Ha-pa c-qd" width="24px" height="24px" viewBox="0 0 24 24" fill="#0F9D58"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"></path></svg>');
                                $('.wpmf-process-file-item[data-cid="'+ folder_name +'"]').removeClass('wpmf-uploading');
                            } else {
                                $('.wpmf-process-file-item[data-cid=' + folder_name + '] .wpmf-fileupload-progress-bar').css({'background': 'radial-gradient(closest-side, white 70%, transparent 80% 100%),    conic-gradient(#0073ab '+ percent +'%, #ddd 0)'});
                            }
        ...
        getUploadItemTemplate: function (id, name, icon) {
...
<div class="ju-settings-option folder_permission_item folder_permission_inherit_folder_item">
         ...
         * @param ids
         */
        deleteMultipleFolders: function (ids) {
            if (ids.length) {
                $.ajax({
                    type: "POST",
                    url: wpmf.vars.ajaxurl,
...
"><circle cx="50" cy="50" ng-attr-r="{{config.radius}}" ng-attr-stroke-width="{{config.width}}" ng-attr-stroke="{{config.stroke}}" ng-attr-stroke-dasharray="{{config.dasharray}}" fill="none" stroke-linecap="round" r="40" stroke-width="12" stroke="#2196f3" stroke-dasharray="62.83185307179586 62.83185307179586" transform="rotate(53.6184 50 50)"><animateTransform attributeName="transform" type="rotate" calcMode="linear" values="0 50 50;360 50 50" keyTimes="0;1" dur="1s" begin="0s" repeatCount="indefinite"></animateTransform></circle></svg></span>`;
    wpmfAddCloudQueue = {
         ...
         */
        addGoogleQueue: function (folder_id = 0) {
            $.ajax({
                method: "POST",
                dataType: "json",
                url: ajaxurl,
