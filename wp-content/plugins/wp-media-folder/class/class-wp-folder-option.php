...
class WpmfMediaFolderOption
{
    ...
    public function exportFolder()
    {
        ...
        if (count($terms) === 0 && count($include_folders) === 0) {
            $post_ids = false;
        } else {
            // get content include
            switch ($export_type) {
                case 'all':
                    $post_ids = $wpdb->get_col($wpdb->prepare('SELECT ID FROM ' . $wpdb->posts . ' WHERE post_type = %s', array('attachment')));
                    break;
                case 'selection_folder':
...
SELECT ID FROM ' . $wpdb->posts . ' as p 
 ...
 INNER JOIN ' . $wpdb->terms . ' AS t ON t.term_id = tt.term_id
  WHERE post_type = %s AND t.term_id IN (%s)
', array('attachment',implode(',', $include_folders))));
                    break;
                case 'only_folder':
                    $post_ids = false;
                    break;
    ...
    public function checkExistPost($file, $termID, $upload_dir)
    {
        ...
        if (empty($termID)) {
            $found = $wpdb->get_var($wpdb->prepare(
                ...
                . ' WHERE guid LIKE %s AND guid LIKE %s AND post_type = %s',
                array('%' . $file, '%' . $ext,'attachment')
            ));
        } else {
            $found = $wpdb->get_var($wpdb->prepare(
                'SELECT COUNT(*) FROM ' . $wpdb->prefix . 'posts as p'
                ...
                . ' WHERE guid LIKE %s AND guid LIKE %s AND post_type = %s'
                . ' AND t1.term_id=%d',
                array('%' . $file . '%', '%' . $ext,'attachment', $termID)
            ));
        }
